...
def test_bad_url_returns_page_not_found(client_request):
    ...
    assert page.title.string.strip() == 'Page not found – Beta Notifications'
...
def test_csrf_returns_400(client_request, mocker):
    ...
    assert page.h1.string.strip() == 'Sorry, there’s a problem with Beta Notifications'
    assert page.title.string.strip() == 'Sorry, there’s a problem with the service – Beta Notifications'
