    ...
    return `<div id="sticky_template_forms">
              ...
              <div id="move_to_folder_radios" class="sticky-template-form" role="region" aria-label="Choose the folder to move selected items to">
                ...
                <div class="js-will-stick-at-bottom-when-scrolling">
                  <div class="page-footer">
                    <button type="submit" class="fr-btn fr-btn--secondary govuk-!-margin-right-3 govuk-!-margin-bottom-1" name="operation" value="move-to-existing-folder">Move</button>
                  </div>
                </div>
              </div>
              <div id="move_to_new_folder_form" class="sticky-template-form" role="region" aria-label="Enter name of the new folder to move selected items to">
                <div class="js-will-stick-at-bottom-when-scrolling">
                  ...
                  <div class="page-footer">
                    <button type="submit" class="fr-btn fr-btn--secondary govuk-!-margin-bottom-1" name="operation" value="move-to-new-folder">Add to new folder</button>
                  </div>
                </div>
              </div>
              <div id="add_new_folder_form" class="sticky-template-form" role="region" aria-label="Enter name of the new folder">
                <div class="js-will-stick-at-bottom-when-scrolling">
                  ...
                  <div class="page-footer">
                    <button type="submit" class="fr-btn page-footer__button" name="operation" value="add-new-folder">Add new folder</button>
                  </div>
                </div>
              </div>
              <div id="add_new_template_form" class="sticky-template-form" role="region" aria-label="Choose template type" ${newTemplateDataModules}>
                ...
                <div class="js-will-stick-at-bottom-when-scrolling">
                  <div class="page-footer">
                    <button type="submit" class="fr-btn page-footer__button" name="operation" value="add-new-template">Continue</button>
                  </div>
                </div>
              </div>
...
describe.skip("TemplateFolderForm", () => {
  const hierarchy = [
    {
      label: "Folder 1",
      type: "folder",
  ...
  beforeEach(() => {
    ...
    templateFolderForm = document.querySelector(
      "form[data-module=template-folder-form]"
    );
  });
  ...
  describe("Before the module starts", () => {
    ...
    test("the HTML for the module should contain placeholder classes on each part that needs to be sticky", () => {
      ...
      expect(
        templateFolderForm.querySelector(
          "#add_new_folder_form > .js-will-stick-at-bottom-when-scrolling"
        )
      ).not.toBeNull();
      expect(
        templateFolderForm.querySelectorAll(
          "#add_new_template_form > .js-will-stick-at-bottom-when-scrolling"
        ).length
      ).toEqual(2);
    });
  });
  ...
  describe("When the module starts", () => {
    ...
    test("the default controls and the counter should be showing", () => {
      expect(
        document.querySelector("button[value=add-new-template]")
      ).not.toBeNull();
      expect(
        document.querySelector("button[value=add-new-folder]")
      ).not.toBeNull();
      expect(
        document
          .querySelector("button[value=add-new-template]")
          .getAttribute("aria-expanded")
      ).toEqual("false");
      expect(
        document
          .querySelector("button[value=add-new-folder]")
          .getAttribute("aria-expanded")
      ).toEqual("false");
    ...
    describe("Selection counter", () => {
      ...
      test("the content of both visible and hidden counters should match", () => {
        expect(visibleCounter.textContent.trim()).toEqual(
          hiddenCounter.textContent.trim()
        );
      });
    });
    ...
    test("should make the current controls sticky", () => {
      ...
      expect(
        formControls.querySelector(
          "#nothing_selected .js-stick-at-bottom-when-scrolling"
        )
      ).not.toBeNull();
      ...
      expect(GOVUK.stickAtBottomWhenScrolling.setMode.mock.calls[0][0]).toEqual(
        "default"
      );
    });
  });
  ...
  describe("Click 'New template' for single channel service", () => {
    test("should redirect to new template page", () => {
      ...
      helpers.triggerEvent(
        formControls.querySelector("[value=add-new-template]"),
        "click"
      );
      // expect url to change
      expect(mockCallback).toHaveBeenCalledWith(
        "/services/123/templates/add-sms"
      );
  ...
  describe("Clicking 'New template'", () => {
    ...
    describe("Should show a region", () => {
      test("with options for all the types of template", () => {
        ...
        const radios = Array.from(
          formControls.querySelectorAll("input[type=radio]")
        );

        options.forEach((option) => {
          let matchingLabels = labels.filter(
            (label) => label.textContent.trim() === option
          );

          expect(matchingLabels.length > 0).toBe(true);

          let matchingRadio = formControls.querySelector(
            `#${matchingLabels[0].getAttribute("for")}`
          );
      ...
      test("with an accessible role, name and description", () => {
        ...
        expect(description.textContent.trim()).toEqual(
          "Press continue to confirm selection or cancel to close"
        );
      });

      test("and focus it", () => {
        expect(document.activeElement).toBe(
          formControls.querySelector("#add_new_template_form")
        );
      });
    });
    ...
    test("should make the current controls sticky", () => {
      ...
      expect(
        formControls.querySelectorAll(
          "#add_new_template_form .js-stick-at-bottom-when-scrolling"
        ).length
      ).toEqual(2);
    ...
    describe("When the 'Cancel' link is clicked after choosing to add a new template", () => {
      ...
      beforeEach(() => {
        ...
        addNewTemplateButton = formControls.querySelector(
  ...
  describe("Clicking 'New folder'", () => {
    ...
    beforeEach(() => {
      ...
      helpers.triggerEvent(
        formControls.querySelector("[value=add-new-folder]"),
        "click"
      );
    ...
    describe("should show a parent region", () => {
      ...
      test("with a textbox for the folder name", () => {
        ...
        expect(
          formControls.querySelector(`label[for=${textbox.getAttribute("id")}]`)
        ).not.toBeNull();
      });
      ...
      test("and focus it", () => {
        expect(document.activeElement).toBe(
          formControls.querySelector("#add_new_folder_form")
        );
      });
    });
    ...
    test("should make the current controls sticky", () => {
      ...
      expect(
        formControls.querySelector(
          "#add_new_folder_form .js-stick-at-bottom-when-scrolling"
        )
      ).not.toBeNull();
    ...
    describe("When the 'Cancel' link is clicked after choosing to add a new folder", () => {
      ...
      beforeEach(() => {
        ...
        addNewFolderButton = formControls.querySelector(
          "button[value=add-new-folder]"
        );
      });
  ...
  describe("When some templates/folders are selected", () => {
    ...
    test("should make the current controls sticky", () => {
      ...
      expect(
        formControls.querySelector(
          "#items_selected .js-stick-at-bottom-when-scrolling"
        )
      ).not.toBeNull();
    ...
    describe("'Clear selection' link", () => {
      ...
      test("clicking the link clears the selection", () => {
        ...
        const checkedCheckboxes = Array.from(templateFolderCheckboxes).filter(
    ...
    describe("Clicking the 'Add to new folder' button", () => {
      ...
      beforeEach(() => {
        ...
        helpers.triggerEvent(
          formControls.querySelector("[value=move-to-new-folder]"),
          "click"
        );
      ...
      describe("Should show a region", () => {
        test("with an accessible role, name and description", () => {
          ...
          expect(region.hasAttribute("aria-label")).toBe(true);
          expect(region.getAttribute("aria-label")).toEqual(
            "Enter name of the new folder to move selected items to"
          );
          expect(region.hasAttribute("aria-describedby")).toBe(true);
          id = region.getAttribute("aria-describedby");
          description = document.getElementById(id);
          expect(description).not.toBeNull();
          expect(description.textContent.trim()).toEqual(
            "Press add to new folder to confirm name or cancel to close"
          );
        });
        ...
        test("with a textbox for the folder name", () => {
          ...
          expect(
            formControls.querySelector(
              `label[for=${textbox.getAttribute("id")}]`
            )
          ).not.toBeNull();
        });
        ...
        test("with a 'Cancel' link", () => {
          ...
          expect(
            cancelLink
              .querySelector(".govuk-visually-hidden")
              .textContent.trim()
          ).toEqual("move to new folder");
        });

        test("and focus it", () => {
          expect(document.activeElement).toBe(
            formControls.querySelector("#move_to_new_folder_form")
          );
        });
      });
      ...
      test("should make the current controls sticky", () => {
        ...
        expect(
          formControls.querySelector(
            "#move_to_new_folder_form .js-stick-at-bottom-when-scrolling"
          )
        ).not.toBeNull();
      ...
      describe("When the 'Cancel' link is clicked after choosing to add a template or folder to a new folder", () => {
        ...
        beforeEach(() => {
          ...
          moveToNewFolderButton = formControls.querySelector(
            "button[value=move-to-new-folder]"
          );
        });
