...
describe.skip("Collapsible fieldset", () => {
  ...
  beforeEach(() => {
    // set up DOM
    document.body.innerHTML = `<div class="selection-wrapper" data-module="collapsible-checkboxes" data-field-label="folder">
  ...
  describe("when started", () => {
    ...
    test("adds the right content and classes to the summary", () => {
      ...
      expect(
        summary.classList.contains("selection-summary__text--folders")
      ).toBe(true);
    });
    ...
    test("has the correct aria attributes on the button", () => {
      expect(
        helpers
          .element(formGroup.querySelector(".govuk-button"))
          .hasAttributesSetTo({
    ...
    describe("the live region that was inside the hint", () => {
      test("is moved above the fieldset", () => {
        expect(
          fieldset.previousElementSibling.matches(".selection-summary")
        ).toBe(true);
      });
      ...
      test("has an id matching the aria-describedby on the fieldset", () => {
        ...
        expect(fieldset.previousElementSibling.getAttribute("id")).toEqual(
          fieldsetDescribedby
        );
      });
    });
  ...
  test("has the right summary text when started with no checkboxes selected", () => {
    ...
    expect(summaryText.textContent.trim()).toEqual(
  ...
  test("the summary doesn't have a folder icon if fields aren't called 'folder'", () => {
    ...
    expect(
      summaryText.classList.contains(".selection-summary__text-label")
    ).toBe(false);
  });
  ...
  describe("when button is clicked while the fieldset is collapsed", () => {
    ...
    test("it uses ARIA to mark the checkboxes as expanded", () => {
      expect(
        formGroup.querySelector(".govuk-button").getAttribute("aria-expanded")
      ).toEqual("true");
    });

    test("it changes it's text to indicate it's new action", () => {
      expect(
        formGroup.querySelector(".govuk-button").textContent.trim()
      ).toEqual("Done choosing folders");
    });
  });
  ...
  describe("when button is clicked when the fieldset is expanded", () => {
    ...
    test("it focuses the summary text", () => {
      expect(document.activeElement).toBe(
        document.querySelector(".selection-summary__text")
      );
    });
  ...
  describe("the footer (that wraps the button)", () => {
    describe("is inserted", () => {
      test("after the fieldset", () => {
        ...
        expect(
          formGroup.querySelector(".selection-footer").previousElementSibling
            .nodeName
        ).toBe("FIELDSET");
      });
      ...
      test("after the root fieldset if the checkboxes are nested", () => {
        ...
        nestedCheckboxes.className =
          "govuk-form-group govuk-form-group--nested";
        nestedCheckboxes.innerHTML = _checkboxes(11, 20);
        checkboxesContainer
          .querySelector(".govuk-checkboxes__item")
          .appendChild(nestedCheckboxes);
    ...
    describe("its stickiness", () => {
      ...
      test("is added when the fieldset is expanded", () => {
        expect(
          formGroup
            .querySelector(".selection-footer")
            .classList.contains("js-stick-at-bottom-when-scrolling")
        ).toBe(true);
        expect(
          window.GOVUK.stickAtBottomWhenScrolling.recalculate.mock.calls.length
        ).toBe(1);
      });
